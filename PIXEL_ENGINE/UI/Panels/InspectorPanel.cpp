...
void InspectorPanel::setSelectedEntity(const EditorEntityView* entity) {
...
void InspectorPanel::draw(const EditorLayout& layout) {
    ...
    const EditorEntityView& entity = *m_selectedEntity;
    const long selectedEntityID = entity.entityID;

    if (m_lastSelectedEntityID != selectedEntityID || !ImGui::IsAnyItemActive()) {
        syncSelectedEntityFields(entity);
        m_lastSelectedEntityID = selectedEntityID;
    }
    ...
    if (ImGui::InputText("##EntityName", m_entityNameBuffer, sizeof(m_entityNameBuffer))) {
        ...
        event.info.entityID = selectedEntityID;
        event.info.name = m_entityNameBuffer;
        EventSystem::pushEvent(event);
    }

    ImGui::TextDisabled("ID  %ld", selectedEntityID);
    ...
    if (ImGui::Combo("##Gizmo", &gizmoIndex, gizmoNames, IM_ARRAYSIZE(gizmoNames))) {
        EditorEvent event;
        if (gizmoIndex == 0) event.type = EditorEventType::SetGizmoTranslate;
        else if (gizmoIndex == 1) event.type = EditorEventType::SetGizmoRotate;
        else if (gizmoIndex == 2) event.type = EditorEventType::SetGizmoScale;
        EventSystem::pushEvent(event);
    }
    ...
    if (ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        auto queueTransformEvent = [this, selectedEntityID](EditorEventType type) {
            EditorEvent event;
            event.type = type;
            event.info.entityID = selectedEntityID;
    ...
    if (ImGui::BeginPopup("AddComponentPopup")) {
        auto queueComponentEvent = [this, selectedEntityID](EditorComponentType componentType) {
            EditorEvent event;
        ...
        if (ImGui::MenuItem("Texture")) {
            if (m_contentBrowser) {
                Logger::addLog(LOG_INFO,
                    "Texture selection mode activated for Entity ID: " + std::to_string(selectedEntityID));
                m_contentBrowser->setTextureSelectionMode(true, selectedEntityID);
            }
        }
        ImGui::EndPopup();
    }
